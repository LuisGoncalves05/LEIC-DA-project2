...
std::pair<std::vector<int>, std::vector<int>> knapsack(std::vector<int> weights, std::vector<int> profits, int max_weight, std::string& algorithm) {
    ...
    if (algorithm == "bf") {
        return knapsack_bf(weights, profits, max_weight, algorithm);
    } else if (algorithm == "dp_iterative") {
        return knapsack_dp_iterative(weights, profits, max_weight, algorithm);
    } else if (algorithm == "dp_recursive_vector") {
        return knapsack_dp_recursive_vector(weights, profits, max_weight, algorithm);
    } else if (algorithm == "dp_recursive_map") {
        return knapsack_dp_recursive_map(weights, profits, max_weight, algorithm);
    } else if (algorithm == "greedy") {
        return knapsack_greedy(weights, profits, max_weight, algorithm);
    } else if (algorithm == "ilp") {
        return knapsack_ilp(weights, profits, max_weight, algorithm);
    } else {
